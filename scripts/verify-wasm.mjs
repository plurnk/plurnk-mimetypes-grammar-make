#!/usr/bin/env node
// CI byte-identical reproducibility check for make.wasm.
...
const committed = await readFile(path.join(repoRoot, "make.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed make.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-make-verify-"));
...
console.log(`rebuilt make.wasm sha256: ${rebuiltHash}`);
