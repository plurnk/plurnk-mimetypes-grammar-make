#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-make.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-make-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter-grammars/tree-sitter-make.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as make.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "make.wasm"));
const bytes = (await readFile(path.join(repoRoot, "make.wasm"))).length;
console.log(`make.wasm: ${bytes} bytes (built from ${pin})`);
